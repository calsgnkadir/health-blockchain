...
For a practice holding therapy records, device theft or master key compromise requires an immediate, deterministic **Key Revocation and Rotation Protocol**.
...
- **Insider Threat / Suspicious Access**: Compromise of a practitioner or administrator account.
...
2. Re-encrypt all client data encryption keys (`DEK`) stored in LMDB metadata:
...
The client or practitioner undergoes physical in-person or out-of-band identity verification to enroll a new hardware Passkey device.
